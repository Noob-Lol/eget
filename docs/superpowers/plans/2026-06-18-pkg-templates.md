...
## Task 4: Config/App Resolution And Template Expansion
...
- [ ] **Step 1: Write merge precedence test**

Add to `internal/config/merge_test.go`:

```go
func TestMergeInstallOptionsWithPackageTemplate(t *testing.T) {
	merged := MergeInstallOptionsWithTemplate(
		Section{Target: stringPtr("~/global")},
		Section{
			LatestURL:   stringPtr("http://mydev.lan/tools/{name}/latest.yaml"),
			URLTemplate: stringPtr("http://mydev.lan/tools/{name}/{name}-{os}-{arch}{ext}"),
		...
		Section{
			...
			URLTemplate: stringPtr("http://override/{name}-{version}{ext}"),
			ExtMap:      map[string]string{"windows": ".zip"},
		},
		CLIOverrides{},
	)

	assert.Eq(t, "~/package", merged.Target)
	assert.Eq(t, "http://mydev.lan/tools/{name}/latest.yaml", merged.LatestURL)
	assert.Eq(t, "http://override/{name}-{version}{ext}", merged.URLTemplate)
	assert.Eq(t, map[string]string{"windows": "win"}, merged.OSMap)
	assert.Eq(t, map[string]string{"amd64": "x64"}, merged.ArchMap)
	assert.Eq(t, map[string]string{"windows": ".zip"}, merged.ExtMap)
}
```

- [ ] **Step 2: Run merge test to verify it fails**

Run:

```bash
go test ./internal/config -run TestMergeInstallOptionsWithPackageTemplate -count=1
```

Expected: compile failure because `MergeInstallOptionsWithTemplate` does not exist.

- [ ] **Step 3: Add template-aware merge helper**

In `internal/config/merge.go`, add:

```go
func MergeInstallOptionsWithTemplate(global, pkgTemplate, pkg Section, cli CLIOverrides) Merged {
	return mergeInstallOptions(global, pkgTemplate, pkg, cli)
}
```

Refactor existing `MergeInstallOptions` to call an unexported helper:

```go
func MergeInstallOptions(global, repo, pkg Section, cli CLIOverrides) Merged {
	return mergeInstallOptions(global, repo, pkg, cli)
}

func mergeInstallOptions(global, repo, pkg Section, cli CLIOverrides) Merged {
	merged := Merged{}
	// move existing body here unchanged
	return merged
}
```

This keeps behavior unchanged while making call sites explicit when the middle layer is a package template, not a legacy repo section.

- [ ] **Step 4: Write install resolution tests**
...
- [ ] **Step 5: Run install resolution tests to verify they fail**

Run:

```bash
go test ./internal/app -run 'TestInstallTargetResolvesPkgTemplateShortAlias|TestInstallTargetResolvesConfiguredPkgTemplatePackageWithOverride' -count=1
```

Expected: target resolution fails or does not render `latest_url`.

- [ ] **Step 6: Implement app resolution helpers**
...
Change `resolveInstallOptionsWithConfig` to receive an explicit middle section:
...
```bash
gofmt -w internal/config/merge.go internal/config/merge_test.go internal/app/install_resolve.go internal/app/install_config_test.go
go test ./internal/config -run TestMergeInstallOptionsWithPackageTemplate -count=1
go test ./internal/app -run 'TestInstallTargetResolvesPkgTemplateShortAlias|TestInstallTargetResolvesConfiguredPkgTemplatePackageWithOverride|TestInstallTargetMergesTemplatePackageOptions' -count=1
```

Expected: PASS.

- [ ] **Step 8: Commit app resolution**

Run:

```bash
git add internal/config/merge.go internal/config/merge_test.go internal/app/install_resolve.go internal/app/install_config_test.go
...
In `internal/app/list.go`, enrich configured list items immediately after they are built from `cfg.Packages`. Add this helper:
...
func resolveListItemPackageTemplate(cfg *cfgpkg.File, item ListItem) ListItem {
	...
	item.Package = mergeListItemPackageTemplate(source, item.Package)
	return item
}

func mergeListItemPackageTemplate(source, pkg cfgpkg.Section) cfgpkg.Section {
	merged := source
	if pkg.Repo != nil {
		merged.Repo = pkg.Repo
	}
	if pkg.Name != nil {
		merged.Name = pkg.Name
	}
	if pkg.Desc != nil {
		merged.Desc = pkg.Desc
	}
	if len(pkg.AssetFilters) > 0 {
		merged.AssetFilters = append([]string(nil), pkg.AssetFilters...)
	}
	if len(pkg.ExtMap) > 0 {
		merged.ExtMap = util.CloneStringMap(pkg.ExtMap)
	}
	return merged
}
```

This helper intentionally uses the same package-private `resolveInstallSourceSection` from `install_resolve.go`, because `list.go` is in the same `app` package.
...
Apply the same enrichment in `findUpdateTarget` / `ListUpdateCandidatesForTargets` paths when an item is created directly from config so update checks pass rendered latest fields.
...
In `docs/TODO.md`, replace the old registry section wording with `pkg_templates` wording and mark only the design/plan as done if implementation is not yet complete. Use this shape:
