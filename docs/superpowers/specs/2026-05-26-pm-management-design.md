...
`docs/TODO.md` 中提出新增 `eget` 自身管理命令 `pm`：

- `pm clean` 清理缓存，默认清理 3 天前的缓存。
- `pm serve` 启动内网 server，分享 package/sdk cache 文件到内网环境，方便多台机器共享下载资源。

本设计把 `pm` 定位为 `eget` 本机运行环境管理命令。它不替代 `install/update/sdk/config`，而是管理这些命令共同依赖的缓存、缓存服务和后续可能扩展的镜像复用机制。
...
- 后台 daemon 生命周期管理。`pm serve` 是前台进程，退出进程即停止服务。

后续可以增加 `pm status`、`pm doctor`、`pm cache ls` 等命令，但不作为当前设计的实现重点。
...
├── status     查看本机 eget 运行环境状态，后续预留
└── doctor     诊断配置、缓存、安装目录，后续预留
```
...
`pm clean --dry-run`、`pm serve /manifest.json` 和后续 `pm cache ls` 都需要同一套扫描结果。建议定义统一内部结构：
...
```bash
eget pm clean
eget pm clean --dry-run
eget pm clean --older 7d
eget pm clean --all
```
...
```bash
eget pm clean --downloads
eget pm clean --api
eget pm clean --sdk-downloads
eget pm clean --sdk-index
eget pm clean --partial
```

组合使用：

```bash
eget pm clean --downloads --sdk-downloads --older 30d
eget pm clean --api --all
eget pm clean --partial --all
```
...
`pm clean` 只删除文件，不直接删除整个缓存根目录。删除文件后可以递归清理空目录，但不能删除 `cache_dir` 本身。
...
`pm serve` 启动一个只读 HTTP 服务，分享本机 `cache_dir` 中的缓存文件。
...
```bash
eget pm serve --host 0.0.0.0 --port 8686 --root all
```
...
| 字段 | 默认 | 说明 |
| --- | --- | --- |
| `cache_mirror_url` | 空 | 局域网 `pm serve` 地址 |
| `cache_mirror_timeout` | `5` | mirror 请求超时秒数 |
| `cache_mirror_fallback` | `true` | mirror 失败后是否回源下载 |

不把 mirror 配置放在 `[pm]` 下，原因是它影响 `install/download/sdk` 的下载行为，属于全局下载策略。
...
`pm clean` 的主要风险是误删。设计上用以下规则降低风险：
...
- 注册 `pm` 命令和子命令。
- 解析 CLI 参数。
- 调用 `cliService.handlePMClean/handlePMServe`。
...
`eget pm clean --sdk-index` 是全局缓存清理的一部分，面向“释放空间”和“清理本机缓存”。

两者可以共存：

- `sdk index clear` 仍保留，适合 SDK 用户明确刷新 index。
- `pm clean --sdk-index` 适合统一清理缓存时使用。

### 与 download/install

`download/install` 不需要知道 `pm clean`。
...
`pm clean/serve` 使用现有 `global.cache_dir`。
...
- `eget pm cache list`：列出缓存文件，支持按 kind 过滤。
- `eget pm status`：展示 config path、cache dir、缓存大小、SDK index 数量、安装记录路径。
- `eget pm doctor`：检查 cache dir 权限、config 加载、7z 可用性、target 是否在 PATH。
- `pm serve --token`：简单 bearer token，适合内网共享时避免无关机器访问。
- `pm clean --json` 和 `pm serve --json-log`：方便脚本集成。
...
- `eget pm clean --dry-run`
- `eget pm clean --older 7d --downloads`
- `eget pm serve --host 127.0.0.1 --port 0`
...
- `README.md`：增加 `pm clean`、`pm serve` 简要说明。
...
1. `pm clean` 立刻解决缓存清理需求。
2. `pm serve` 先提供人工可用的内网只读服务。
...
1. `pm serve` 默认监听 `0.0.0.0:8686`。理由是该命令的核心目标就是内网共享；安全提示和 `--host 127.0.0.1` 覆盖本机调试场景。
2. `pm clean` 大量删除在 TTY 下需要确认，`--yes` 跳过确认；非 TTY 下如果触发大量删除阈值且没有 `--yes`，直接返回错误。这样兼顾交互安全和脚本可预测性。
3. Phase 4 默认不做 API cache mirror。自动 mirror 先覆盖普通下载缓存和 SDK 下载缓存；API cache 涉及 token、TTL 和 provider 状态，除非后续确有强需求，否则不纳入自动复用。

## 自查

- 没有把 `pm` 设计成第二套 package/sdk 管理入口。
- `pm clean` 默认行为保守，不默认清理 SDK index。
- `pm serve` 从第一期就有 manifest schema，后续自动 mirror 不需要推翻服务端协议。
