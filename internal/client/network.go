...
func DownloadFile(rawURL, target string, getbar func(size int64) io.Writer, opts Options) error {
	if isLocalFile(rawURL) {
	...
	if partSize > 0 && canResumeDownloadFile(rawURL, partSize, metaPath, remote) {
		...
		if resp.StatusCode == http.StatusPartialContent {
			...
			if remote.Size > 0 && fileSize(partPath) != remote.Size {
				return fmt.Errorf("download size mismatch: expected %d, got %d", remote.Size, fileSize(partPath))
			}
			if err := replaceFile(partPath, target); err != nil {
				return err
			}
